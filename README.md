...
открыт главный экран приложения. В исходной версии Android 5.0 добавлены два виджета главного
...
В меню добавления доступны предпросмотры обоих вариантов. В виджете значков
иконки подстраиваются под высоту виджета почти без пустого пространства; если
аккаунты не помещаются, виден счётчик `+N`. Фон вокруг кругов полупрозрачный,
а внутри кругов — непрозрачный. Ширину виджета можно изменить на главном экране.
Кнопка меню справа вверху главного экрана содержит действия **Добавить аккаунт**,
**Обновить**, **Настройки** и **Убрать аккаунт**. При удалении сначала выберите
аккаунт, затем подтвердите действие. На пустом экране показана подсказка, где
найти добавление аккаунта.
Виджеты используют последний сохранённый результат и запрашивают новые лимиты
с интервалом, выбранным в разделе **Настройки → Частота обновления виджетов**.
По умолчанию — 30 минут; доступны интервалы от 15 минут до 24 часов. Android
может задерживать фоновые обновления для экономии батареи; открытый главный экран
приложения обновляет виджеты сразу.
...
после обновления стоит проверить вход и получение лимитов на настоящем телефоне.
Кнопка загружает предварительный релиз `v5.3` с виджетами. APK подписан тем же
отладочным ключом, что и `v4.4`; его можно установить поверх предыдущей сборки
без удаления аккаунтов. Работу виджетов на телефоне после изменения размера
индикаторов ещё не проверяли.
...
отладочный APK из текущих исходников в `dist/AIUsageMonitor-Android_v5.3-debug.apk`;
это не релизная подпись.
